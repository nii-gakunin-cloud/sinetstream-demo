    ...
    curl -f -s -S -X POST -H "Content-Type: application/json" "http://localhost:$PORT/connectors" -d@- <<EOF >/dev/null
{
  ...
  "config": {
    ...
    "auto.create": true,
    "transforms": "Timestamp,ByteArrayCast,Etag",
    ...
    "transforms.ByteArrayCast.field":"msg",
    "transforms.Etag.type": "jp.ad.sinet.sinetstream.connect.transforms.CalcEtag\$Value",
    "transforms.Etag.field": "msg",
